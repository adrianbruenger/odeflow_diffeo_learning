...
def get_LASA_data(data_name, device, n_samples=None):
    ...
    X0_test = x_train_tensor[Lasa.idx[:-1],:]#.unsqueeze(0) # Lasa.idx[:-1]
...
def train(learning_model, tensor_dataset, n_points, dt, optimizer, loss_fcn, X0_test, data_test, \
    epochs=3000, batch_size=None, stop_threshold=1000, \
    log_frq=5, test_frq=100, vis_frq=100, device="cpu", name = ""
    ):
    ...
    # progress bar setup
    progress_bar = tqdm(total=epochs, desc="Training progress", position=0, leave=False)
    ...
    for epoch in range(1,epochs+1):
        ...
        if loss_list[epoch] < min(loss_list[:-1]):
            ...
            torch.save(best_model, f"best_NODE_model_{name}")
        ...
        # update progress bar
        progress_bar.set_postfix_str(f"Current MSE loss: {training_loss:.4f}, \
...
def main(name):
    ...
    ## LASA data
    X0_test, data_test, tensor_dataset, dt = get_LASA_data(name, device, n_points)
    ...
    optimizer = torch.optim.Adam(learning_model.parameters(), lr=0.001)
    loss_fcn  = torch.nn.MSELoss()

    print(f"Training {name}")
    loss_list = train(learning_model, tensor_dataset, n_points*7, dt, optimizer, loss_fcn, X0_test, data_test, \
        epochs=epochs, batch_size=batch_size, stop_threshold=1000, \
        log_frq=log_frq, test_frq=test_frq, vis_frq=vis_frq, device=device, name=name
        )
